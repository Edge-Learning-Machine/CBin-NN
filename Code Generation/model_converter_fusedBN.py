...
with lq.context.quantized_scope(True):
    for layer_idx, layer in enumerate(model.layers):
        ...
        if 'conv' in layer.name:
            ...
            # Quantized weights for the first layer 
                ...
                if layer_idx == 0 and loop_unrolling == True and 'p_re_lu' in model.layers[layer_idx+1].name and ker_size != 1:
                    ...
                    layer_name = 'QQConv2D'
                # In case the convolutional weights are quatized (not binary) the BN layer can be fused directly in the convolutional weights
                # Update save_params.py accordingly (remove alpha1_bn and alpha2_bn in QQConv2D operators)
                BN_fused = True 
                if 'batch' in model.layers[layer_idx+1].name:
                    BN_layer = model.layers[layer_idx+1]
                else:
                    BN_layer = model.layers[layer_idx+2]
                BN_weights = BN_layer.get_weights()
                gamma= BN_weights[0].reshape((1,1,1,BN_weights[0].shape[0]))
                beta = BN_weights[1]
                mean = BN_weights[2]
                variance = BN_weights[3].reshape((1,1,1,BN_weights[3].shape[0]))
                epsilon = BN_layer.epsilon
                weights = weights * gamma / np.sqrt(variance * epsilon)
                weights = weights.transpose(3,0,1,2)
                weights = quantize_weights(weights)
                if layer.bias != None:
                    bias = layer.get_weights()[1]
                    bias = beta + (bias - mean) * gamma / np.sqrt(variance * epsilon)
                    bias = quantize_weights(bias)
                else:
                    bias = None
                    
            sp.save_bnn_params(weights, bias, layer_name, Conv_idx, in_shape, out_shape, ker_size, stride, padding, first_file_write, in_buffer, out_buffer)
            Conv_idx += 1
            ...
            sp.save_bnn_params(weights, bias, layer_name, FC_idx, in_dim, out_dim, NULL, NULL, NULL, first_file_write, in_buffer, out_buffer)
            FC_idx += 1
            first_file_write = False
            if in_buffer == 'buffer1':
                in_buffer = 'buffer2'
            ...
            # In case the first convolutional layer is quantized BN layer can be fused within the convolutional weights
            # The other BN layers needs special handling since they cannot be fused within the convolutional weights 
            # (binary weights and float BN params)
            if BN_fused == True:
                BN_fused = False
                continue
                ...
                if BN_idx == 1:
                    ...
                    sp.save_bnn_params(alpha, NULL, layer_name, BN_idx, dim, NULL, NULL, NULL, NULL, first_file_write, NULL, NULL)
                    BN_idx = BN_idx + 1
                # All other BN layers are handled as follows, for more information refer to the paper
                    ...
                    sp.save_bnn_params(integer_bias, NULL, layer_name, BN_idx, dim, NULL, NULL, NULL, NULL, first_file_write, NULL, NULL)
                    BN_idx = BN_idx + 1
            first_file_write = False
            ...
            sp.save_bnn_params(weights, NULL, layer_name, PReLU_idx, dim, NULL, NULL, NULL, NULL, first_file_write, NULL, NULL)
            ...
            sp.save_bnn_params(NULL, NULL, layer_name, MaxPool_idx, in_shape, out_shape, pool_size, stride, padding, first_file_write, in_buffer, out_buffer)
        ...
        ### To be tested 
            ...
            sp.save_bnn_params(NULL, NULL, layer_name, NULL, NULL, n_classes, NULL, NULL, NULL, first_file_write, NULL, NULL)
            first_file_write = False
...
sp.save_bnn_params(NULL, NULL, layer_name, NULL, max_activation, second_max_activation, NULL, NULL, NULL, first_file_write, NULL, NULL)